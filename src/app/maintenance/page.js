...
export default function MaintenancePage() {
  ...
  async function fetchRecords() {
    if (!email) return;
    const res  = await fetch("/api/maintenance", { headers: { "x-user-email": email }, cache: "no-store" });
  ...
  async function handleBikeSearch() {
    ...
    if (!make.trim() && !model.trim()) { alert("Enter a Make or Model to search."); return; }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { alert(data.error || "Bike search failed"); return; }
      setBikeResults(Array.isArray(data) ? data : []);
      if (!data?.length) alert("No bikes found.");
    } catch { alert("Bike search error."); }
  ...
  function toggleTask(task) {
    setForm((prev) => ({
      ...prev,
      type: prev.type.includes(task) ? prev.type.filter((t) => t !== task) : [...prev.type, task],
    }));
  }
